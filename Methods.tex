...
Se pretende agrupar taxones en grupos que reflejen su comportamiento frente a una perturbación (viaje o salmonelosis). Para ello, se desarrolló un método sencillo dividiendo las tablas de abundancia relativa en 3 periodos: antes ($a$), durante ($d$) y tras ($r$) el punto en cuestión. Para cada taxón, se calculó la mediana de su abundancia en cada uno de estos periodos ($M_a$, $M_d$, $M_t$). Por último, se calculan las diferencias $M_a - M_d$ y $M_t - M_d$. Si el primer incremento es positivo, quiere decir que la abundancia ha disminuído con la perturbación y si el incremento es negativo, quiere decir que ha aumentado. En el segundo incremento es lo contrario, valores positivos indican disminución de abundancia y valores negativos indican aumento. Además, se escoge un valor de $0.01$ para que el incremento se considere significativamente grande como para que haya cambio en la abundancia. A continuación se especifican los 7 grupos que se han considerado:

\begin{table}[H]
\centering
\begin{tabular}{ >{\centering\arraybackslash}m{8cm} >{\arraybackslash}m{5cm}}
\hline
 & Grupos \\
\hline \hline
\multirow{7}{1cm}{}& Grupo1 \\ \cline{2-2}
%\includegraphics[width=50mm]{./Figuras/StoolA_G2}
& Grupo2 \\ \cline{2-2}
& Grupo3 \\  \cline{2-2}
& Grupo4 \\ \cline{2-2}
& Grupo5 \\ \cline{2-2}
