...
NOTA: es muy importante aquí usar -n porque le dice al \textit{script} que descarte todas las secuencias que le hemos pasado a través de -s, es decir, que elimine las quimeras. Esto se hace así porque el \textit{script identify\_chimeric\_seqs.py} ofrece una lista de las secuencias quiméricas y no de las secuencias no quiméricas.

\subsection*{ 4.1 Eliminar ficheros con bajo número de lecturas}
Se hizo un conteo del número de \textit{reads} por fichero tras todo el preprocesado y se guardó en un fichero adicional:
\begin{listing}[style=consola, numbers=none]
...
\begin{description2}
    ...
    \item  -p : parámetos para introducir en los distintos pasos del script. 
\end{description2}

En este caso se utiliza como parámetros un fichero auxiliar llamado "uc\_fast\_params.txt" con el siguiente contenido:
\begin{lstlisting}[style=Python]
pick_otus:enable_rev_strand_match True
\end{lstlisting}

Esta instrucción sirve para que tenga en cuenta ambos sentidos de lectura de la secuencia cuando haga el "pick otus".
...
Además, para pasos posteriores se necesita subdividir esa tabla grande final en subtablas que abarcan distintos intervalos de tiempo. Todo ello lo hace un \textit{script} implementado en python. Se muestra como ejemplo el \textit{script} de saliva A (los \textit{scripts} para intestino A e intestino B pueden encontrarse en el material suplementario con los nombres "merge\&split\_stoolA.py" y "merge\&split\_stoolB.py", respectivamente):
